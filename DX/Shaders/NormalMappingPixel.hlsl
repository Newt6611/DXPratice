...
{
	float3 normal = normalTexture.Sample(samplerState, ps_in.texcoord).xyz;
	normal = normal * 2.0 - 1.0;
	normal = normalize(mul(ps_in.TBN, normal));

	//float4 result = DoDirectionalLight(ps_in.fragPos, normal, ps_in.texcoord);
